...
def start_screen_capturing():
    ...
    Statusbar['text']='"Screen Recording started..."'
...
def merge_all():
  ...
  Statusbar['text']='"Video saved sucessfully/"'
# --- webcam
...
def take_shot():
  ...
  date=datetime.now()
  img.save('img_%s%s%s%s%s%s.png'%(date.year,date.month,date.day,date.hour,date.minute,date.second))
  root.update()
  Statusbar['text']='"Screenshot Saved"'
  root.deiconify()
...
root = tk.Tk()
img = PhotoImage(file=f'C://Users/{os.getlogin()}/desktop/pyrec/monitor.png')
root.tk.call('wm', 'iconphoto', root._w, img)
root.title('                                                                                                                        Screen Recorder')
root.geometry('+260+70')
root.resizable(width=False,height=False)
st_icon = PhotoImage(file=f'C://Users/{os.getlogin()}/desktop/pyrec/rec.png')
sp_icon = PhotoImage(file=f'C://Users/{os.getlogin()}/desktop/pyrec/stop.png')
cam_icon = PhotoImage(file=f'C://Users/{os.getlogin()}/desktop/pyrec/webcam0.png')
com_icon = PhotoImage(file=f'C://Users/{os.getlogin()}/desktop/pyrec/webcam1.png')
shot = PhotoImage(file=f'C://Users/{os.getlogin()}/desktop/pyrec/screenshot.png')
...
short_b = tk.Button(frame,image=shot,text='Screenshot',compound="top",relief=FLAT , width=70, command=take_shot)
short_b.grid(row=0, column=0,padx=80,pady=10)

webcam_btn = tk.Button(frame, image=cam_icon,text='Webcam',compound="top",relief=FLAT , width=70,height= 80,command=change_w)
webcam_btn.grid(row=0,column=2,padx=80,pady=10)
webcam_btn.image = cam_icon

main_btn = tk.Button(frame, image=st_icon,text='Record',compound="top",relief=FLAT , width=70,height=80, command=change_icon)
main_btn.grid(row=0,column=1,padx=80,pady=10)
main_btn.image = st_icon

Statusbar = Label(frame,text="Ready to record",font='times 12 ',fg='red',relief=FLAT)
Statusbar.grid(row=1,column=1,ipadx=90)
